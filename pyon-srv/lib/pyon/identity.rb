# pyon/identity.rb
# Lê ~/.pyon/identity.json produzido pelo pyon-core (Rust).
...
module Pyon
  class Identity
    ...
    def initialize(path = nil)
      path ||= File.join(Dir.home, ".pyon", "identity.json")
      raise "identidade não encontrada em #{path} — rode pyon-core primeiro" unless File.exist?(path)
