  ...
  class RelayClient
    ...
    def run
      begin
        ...
        $stderr.puts "[pyon relay] falha ao conectar em #{@host}:#{@port} — #{e.message}"
        return
      rescue => e
        emit("error", { "msg" => "erro de rede: #{e.message}" })
        $stderr.puts "[pyon relay] erro: #{e.message}"
        return
      end
      $stderr.puts "[pyon relay] conectado a #{@host}:#{@port}"
    ...
    def save_post_locally(payload)
      ...
      db_dir = File.join(Dir.home, ".pyon", "db")
...
if __FILE__ == $0
  ...
  client = Pyon::RelayClient.new(host: host, port: port, channel: channel,
                                     identity: identity)
  client.run
end
