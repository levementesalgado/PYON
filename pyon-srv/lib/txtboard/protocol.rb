...
module Txtboard
  module Protocol
    ...
    def self.build_signed(type, payload, identity)
      # pubkey deve estar no payload ANTES de calcular a canonical —
      # verify! também inclui pubkey ao recalcular, então devem ser idênticos
      full_payload = payload.merge("pubkey" => identity.pubkey_hex)
      canonical = canonical_bytes(type, full_payload)
      sig       = identity.sign(canonical)
      build(type, full_payload.merge("sig" => sig))
    end
