...
Coloque o `relay_client.rb` em `/usr/local/share/pyon/` ou ajuste
o caminho em `relay.c` (linha `snprintf(cmd, ...)`).

```bash
# estrutura esperada
/usr/local/share/pyon/relay_client.rb

# ou rode apontando para o fonte diretamente:
# edite relay.c: troque o caminho por ./pyon-srv/lib/pyon/relay_client.rb
```

## config (~/.pyon/config.json)
...
A identidade é lida automaticamente de `~/.pyon/identity.json`
(gerado pelo `pyon-core` em Rust).
