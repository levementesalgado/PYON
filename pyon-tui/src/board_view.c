...
static void load_ndjson(const char*slug){
    ...
    char path[256];
    snprintf(path,sizeof(path),"%s/.pyon/db/posts.ndjson",home);
...
static void thread_view_run(const Board*b,const Config*cfg,int root_id){
    ...
    while(1){
        ...
        /* busca */
        if(srching){
            if(ch==27||IS_ENTER(ch)){srching=0;srch[0]=0;}
...
void board_view_run(const Board*b,const Config*cfg){
    ...
    while(1){
        ...
        if(srching){
            if(ch==27||IS_ENTER(ch)){if(ch==27){srch[0]=0;build_filter("");}srching=0;sel=0;scroll=0;}
