...
static void draw_boards(int y0,int x0,int w,int h,int sel_fi,int scroll_line){
    ...
    for(int fi=0;fi<flt_n;fi++){
        ...
        int right_edge = x0+w-2;  /* coluna máxima permitida */
        ...
        if(cur>=scroll_line&&scr_row<max_row){
            ...
            attrset(A_NORMAL);
            /* slug — limitado a 9 chars */
            if (is_sel) attron(COLOR_PAIR(C_HEADER)|A_BOLD);
            else        attron(cat_attr(b->cat));
            mvprintw(scr_row,x0+5,"  /%-7.7s",b->slug);
            attrset(A_NORMAL);
            /* título — largura calculada para não ultrapassar borda */
            int title_x = x0+15;
            int tw = right_edge - title_x;
            if(tw > 0){
                attron(is_sel?(COLOR_PAIR(C_HEADER)|A_BOLD):COLOR_PAIR(C_NORMAL));
                mvprintw(scr_row,title_x,"%-*.*s",tw,tw,b->title);
                attrset(A_NORMAL);
            }
            scr_row++;
        }
        ...
        /* linha 2: tags — acumula em buffer e trunca antes de printar */
        if(cur>=scroll_line&&scr_row<max_row){
            char tagbuf[128]={0}; int tpos=0;
            for(int t=0;t<3;t++){
                if(!b->tags[t][0]) break;
                int needed=(int)strlen(b->tags[t])+2;
                if(tpos+needed < (int)sizeof(tagbuf)-1){
                    tpos += snprintf(tagbuf+tpos,sizeof(tagbuf)-tpos," #%s",b->tags[t]);
                }
            }
            int tag_x=x0+15;
            int tw2=right_edge-tag_x;
            if(tw2>0 && tagbuf[0]){
                attron(is_sel?COLOR_PAIR(C_HEADER):(COLOR_PAIR(C_DIM)|A_BOLD));
                mvprintw(scr_row,tag_x,"%.*s",tw2,tagbuf);
                attrset(A_NORMAL);
            }
            scr_row++;
        }
...
int home_run(Config *cfg) {
    ...
    while(1){
        ...
        int h=rows-5; if(h<6)h=6;  /* linha 0=topbar, 1=hint, 2..rows-3=box, rows-2=rodapé */
        int y0=2;
        if(y0+h > rows-2) h = rows-2-y0;
        if(h<4) h=4;
        ...
        /* topbar: kaomoji + nome + status relay à direita */
        char tl[80], tr[48];
        snprintf(tl,sizeof(tl),"✦ PYON  %s  %s",greets[gi],cfg->my_name);
        if(relay_connected()) snprintf(tr,sizeof(tr),"relay ✓  v0.2");
        else                  snprintf(tr,sizeof(tr),"offline  v0.2");
        ui_topbar(cols,tl,tr);

        /* hint na linha 1 — sempre em col 2 para ficar visível */
        if(searching){
            attron(COLOR_PAIR(C_WARN)|A_BOLD);
            mvhline(1,0,' ',cols);
            mvprintw(1,2,"/ %s_  [%d boards]",srch,flt_n);
            attroff(COLOR_PAIR(C_WARN)|A_BOLD);
        } else {
            attron(COLOR_PAIR(C_DIM)|A_BOLD);
            mvhline(1,0,' ',cols);
            mvprintw(1,2,"[↑↓/jk] nav  [Enter] abrir  [/] buscar  [p] perfil  [r] relay  [q] sair  [Enter] ok");
        ...
        int ch = (tick % 10 == 0) ? ERR : ui_key_timeout(3);
        tick = (tick % 10 == 0) ? 0 : tick;

        if(ch==ERR){ relay_tick(); continue; }

        if(searching){
            if(ch==27||IS_ENTER(ch)){
        ...
        switch(ch){
            ...
            case'p':{
                /* abre página de perfil completa */
                profile_edit(cfg);
                gi=(int)(time(NULL)%5);
                clear(); refresh();
                break;
