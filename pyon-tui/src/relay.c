...
/* Enter: \n(10) \r(13) KEY_ENTER — compatível com VT e Konsole */
#define IS_ENTER(c) ((c)==10||(c)==13||(c)==KEY_ENTER)
/* ── constantes ──────────────────────────────────────────────── */
#define MAX_MSGS     512
...
static char g_relay_host[128]= {0};
...
/* wrapper público — usado por profile.c e outros módulos */
void relay_send(const char *json) { send_relay(json); }
...
static int spawn_relay(const char *host, int port, const char *channel) {
    ...
    if (pid == 0) {
        ...
        snprintf(cmd, sizeof(cmd),
            "SELF=$(readlink -f /proc/self/exe 2>/dev/null || echo ''); "
            "SELFDIR=$(dirname \"$SELF\" 2>/dev/null); "
            "RC=\"\"; "
            "for p in"
            " /usr/local/share/pyon/lib/pyon/relay_client.rb"
            " /usr/share/pyon/lib/pyon/relay_client.rb"
            " \"$HOME/.local/share/pyon/lib/pyon/relay_client.rb\""
            " \"$SELFDIR/../share/pyon/lib/pyon/relay_client.rb\""
            " \"$(find /usr/local/share /usr/share 2>/dev/null"
            "   -name relay_client.rb -path '*/pyon/*' 2>/dev/null | head -1)\""
            "; do"
            "  [ -f \"$p\" ] && RC=\"$p\" && break; "
            "done; "
            "if [ -z \"$RC\" ]; then"
            "  echo '{\"type\":\"error\",\"msg\":\"relay_client.rb nao encontrado — rode setup.sh\"}'; exit 1; "
            "fi; "
            /* torsocks só para endereços .onion — 127.x conecta direto */
            "case '%s' in"
            "  *.onion) exec torsocks ruby \"$RC\" '%s' '%s' '%s' '%s';;"
            "  *)       exec ruby \"$RC\" '%s' '%s' '%s' '%s';;"
            "esac",
            host,
...
void relay_connect(const char *my_name, const char *my_pubkey,
    ...
    if (host)      snprintf(g_relay_host, sizeof(g_relay_host),"%s", host);
    g_relay_port   = port;
    g_reconnect_at = 0;
...
void relay_tick(void) {
    ...
    if (relay_pid > 0) {
        ...
        if (r == relay_pid) {
            /* filho morreu — limpa estado */
            if (relay_in)  { fclose(relay_in);  relay_in  = NULL; }
            if (relay_out) { fclose(relay_out); relay_out = NULL; }
            relay_pid  = -1;
            pipe_in_fd = -1;
            g_connected = 0;
            push_sys("*pip...* conexão perdida — reconectando em 5s… (｡•́︿•̀｡)");
            g_reconnect_at = time(NULL) + 5;
        }
    }

    /* reconexão automática após backoff */
    if (!g_connected && g_reconnect_at > 0 &&
        time(NULL) >= g_reconnect_at &&
        g_relay_host[0] && g_relay_port > 0) {
        g_reconnect_at = 0;
        push_sys("*kyaa~!* tentando reconectar…");
        if (spawn_relay(g_relay_host, g_relay_port, g_channel) == 0) {
            g_connected = 1;
            push_sys("*pyon!* reconectado! (◕‿◕✿)");
        } else {
            push_sys("*pip...* falha — tentará novamente em 10s");
            g_reconnect_at = time(NULL) + 10;
        }
    }
}
...
static void draw_input(WINDOW *iw, int icols, const char *buf,
    ...
    wattron(iw, COLOR_PAIR(C_DIM));
    mvwprintw(iw, 0, icols-28, "Shift+Enter para enviar");
    wattroff(iw, COLOR_PAIR(C_DIM));
...
void relay_run(const char *my_name, const char *my_pubkey,
    ...
    while (1) {
        ...
        } else if (sidebar_focus && (IS_ENTER(ch))) {
        ...
        /* 'p' no sidebar: abre perfil do usuário selecionado */
        } else if (sidebar_focus && ch=='p') {
            if (view_mode==0 && user_sel < user_count) {
                User *u = &users[user_sel];
                /* Config local para passar ao profile_view */
                Config tcfg; memset(&tcfg,0,sizeof(tcfg));
                snprintf(tcfg.my_name,  sizeof(tcfg.my_name),  "%s", my_name?my_name:"");
                snprintf(tcfg.my_pubkey,sizeof(tcfg.my_pubkey),"%s", my_pubkey?my_pubkey:"");
                profile_view(u->pubkey, u->name, &tcfg);
                clear(); refresh();
            }
        ...
        /* input de texto */
            ...
            if (ch == KEY_LEFT) {
                ...
                if (!input[0]) {
                    push_sys("✦ Shift+Enter para enviar ✦");
                } else {
                    /* ── comando /dm nick [msg] ── */
